...
In order to accomplish these goals, I will use the Death Star attack scene during the Battle of Yavin as seen in Star Wars: A New Hope – Episode 4. There are two scenarios (described below) I will use to  demonstrate concepts that have been learned in this class, and may be taught to other audiences.
...
# Body
In order to run this RMarkdown, the following files will need to be accesible:
...
barplot (DeathStarShot$`Hit or Miss`, DeathStarShot$Attempts)
```
...
Because we are dealing with a "one in a million shot" scenario, this should immediately look odd compared to other data sets we have reviewed. For example here is a visualization of normal distribution, not the contrast:
...
```{r}
#Here lets run a test where we want to be 100% certain that the shot is succesful on the first attempt:
...
#Lets modify this once more to modify or certainy level from 99% to 99.9%:
...
First, let us explore what the chances of survival were regardless of the class of Starfighter flown during the Battle of Yavin.
